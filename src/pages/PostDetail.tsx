  ...
  useEffect(() => {
    const fetchPost = async () => {
      ...
      try {
        ...
        setPost(postData);
        setLikeCount(Math.floor(Math.random() * 50) + 10); // Placeholder engagement

        // Fetch author information
        if (postData.userId) {
          try {
            const userProfileResult = await getUserProfile(postData.userId);
            if (userProfileResult.success && userProfileResult.profile) {
              ...
              setAuthor({
                id: postData.userId,
                name: 'Anonymous User',
                image: getAvatarUrl({ username: postData.userId }),
                isVerified: false
              });
            }
          } catch (error) {
